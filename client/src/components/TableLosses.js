...
// Import custom files
import { COLUMNS_LOSSES } from "../config/columnsLosses";
import { selectLosses } from "../redux/slices/chartSlice";
...
function TableLosses() {
  ...
  // Debug
  //console.log("State: ", losses?);

  // Memorize data and columns to avoid re-render of data
  const columns = useMemo(() => COLUMNS_LOSSES, []);
  const data = useMemo(() => {
    // Check data
    if (!losses) {
      return [];
